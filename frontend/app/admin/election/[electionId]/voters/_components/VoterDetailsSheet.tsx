        ...
        >
          <SheetHeader className="p-6 pb-2">
            <div className="flex items-center gap-2 mb-2">
              <Badge variant="outline" className="bg-primary/5 text-primary border-primary/20 flex gap-1.5 items-center px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider">
              ...
              <Badge
                variant="outline"
                className={cn(
                  "py-0.5 px-2 text-[10px] font-bold uppercase tracking-wider pointer-events-none",
                  hasVoted
            ...
            <SheetTitle className="font-semibold text-xl tracking-tight">{voter.name}</SheetTitle>
            <SheetDescription className="text-sm text-muted-foreground leading-relaxed">
              Detailed profile for this registered election voter.
            </SheetDescription>
          </SheetHeader>
          ...
          <div className="flex-1 overflow-y-auto px-6 space-y-6 py-6">
            ...
            <div className="space-y-4">
              <h4 className="text-[10px] uppercase font-bold text-muted-foreground tracking-widest px-1">Integrity Logs</h4>
              <div className="grid gap-3">
                ...
                {voter.createdBy && (
                  <div className="flex items-center gap-4 rounded-xl border bg-card p-4 transition-all hover:bg-muted/10 group relative overflow-hidden">
                    ...
                    <Avatar className="h-10 w-10 shadow-sm border border-border/50 shrink-0 group-hover:scale-105 transition-transform duration-300 font-bold">
                      <AvatarImage src={voter.createdBy.image || ""} alt={voter.createdBy.name || "User"} className="object-cover" />
                      <AvatarFallback className="bg-green-500/5 text-green-600 text-[10px] font-bold">
                        {voter.createdBy.name?.charAt(0) || voter.createdBy.email?.charAt(0) || "U"}
                      </AvatarFallback>
                    </Avatar>
                    <div className="flex-1 min-w-0 text-left relative z-10">
                      <p className="text-xs text-muted-foreground font-medium">Registered By</p>
                      <p className="text-sm font-bold truncate leading-tight mt-0.5 group-hover:text-foreground transition-colors">
                        {voter.createdBy.name || "System Admin"}
                      </p>
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <p className="text-[10px] text-muted-foreground truncate max-w-[140px] block opacity-70">{voter.createdBy.email}</p>
                        </TooltipTrigger>
                        <TooltipContent side="bottom" className="text-xs">
                          {voter.createdBy.email}
                        </TooltipContent>
                      </Tooltip>
                    </div>
                    <div className="text-right flex flex-col items-end gap-1 shrink-0 relative z-10">
                      <Badge variant="secondary" className="font-mono text-[8px] py-0 px-1.5 uppercase tracking-tighter bg-green-500/10 text-green-600 border-none">
                        Creator
                      </Badge>
                      <p className="text-[10px] text-muted-foreground font-medium whitespace-nowrap">
                        {voter.createdAt ? format(new Date(voter.createdAt), "MMM d, h:mm a") : "Unknown"}
                      </p>
                    </div>
                  </div>
                )}
                ...
                {voter.updatedBy && (
                  <div className="flex items-center gap-4 rounded-xl border bg-card p-4 transition-all hover:bg-muted/10 group relative overflow-hidden">
                    ...
                    <Avatar className="h-10 w-10 shadow-sm border border-border/50 shrink-0 group-hover:scale-105 transition-transform duration-300 font-bold">
                      <AvatarImage src={voter.updatedBy.image || ""} alt={voter.updatedBy.name || "User"} className="object-cover" />
                      <AvatarFallback className="bg-purple-500/5 text-purple-600 text-[10px] font-bold">
                        {voter.updatedBy.name?.charAt(0) || voter.updatedBy.email?.charAt(0) || "U"}
                      </AvatarFallback>
                    </Avatar>
                    <div className="flex-1 min-w-0 text-left relative z-10">
                      <p className="text-xs text-muted-foreground font-medium">Last Modified</p>
                      <p className="text-sm font-bold truncate leading-tight mt-0.5 group-hover:text-foreground transition-colors">
                        {voter.updatedBy.name || "System Admin"}
                      </p>
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <p className="text-[10px] text-muted-foreground truncate max-w-[140px] block opacity-70">{voter.updatedBy.email}</p>
                        </TooltipTrigger>
                        <TooltipContent side="bottom" className="text-xs">
                          {voter.updatedBy.email}
                        </TooltipContent>
                      </Tooltip>
                    </div>
                    <div className="text-right flex flex-col items-end gap-1 shrink-0 relative z-10">
                      <Badge variant="secondary" className="font-mono text-[8px] py-0 px-1.5 uppercase tracking-tighter bg-purple-500/10 text-purple-600 border-none">
                        Modified
                      </Badge>
                      <p className="text-[10px] text-muted-foreground font-medium whitespace-nowrap">
                        {voter.updatedAt ? format(new Date(voter.updatedAt), "MMM d, h:mm a") : "Unknown"}
                      </p>
                    </div>
                  </div>
                )}
