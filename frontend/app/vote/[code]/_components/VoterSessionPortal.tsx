...
import { verifyVoterUniqueIdAction } from "@/lib/actions/vote-actions"
import { z } from "zod"
...
export function VoterSessionPortal({ election }: VoterSessionPortalProps) {
    ...
    useEffect(() => {
        if (!isFullscreen && (isIdDialogOpen || isVoterInfoDialogOpen)) {
    ...
    const onIdSubmit = (values: z.infer<typeof VoterIdSchema>) => {
        ...
        startTransition(async () => {
            const result = await verifyVoterUniqueIdAction(election.id, values.uniqueId)
            if (result.error) {
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-blue-50/80 via-background to-primary/5 flex flex-col items-center justify-center p-6 relative overflow-hidden font-sans">
            ...
            {/* Main Content */}
            <div className="max-w-4xl w-full flex flex-col items-center space-y-16 z-10 -mt-24">
