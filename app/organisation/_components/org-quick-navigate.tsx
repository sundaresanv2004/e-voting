...
interface OrgQuickNavigateProps {
...
const navItems = [
  {
    key: "elections",
    label: "Elections",
    description: "Manage campaigns & active polls",
    href: "/organisation/elections",
    icon: MapsIcon,
  ...
  },
  {
    key: "members",
    label: "Members",
    description: "Team roles & access control",
  ...
  },
  {
    key: "logs",
    label: "Audit Logs",
    description: "Review admin activity & security",
  ...
  },
  {
    key: "settings",
    label: "Settings",
    description: "Global organization preferences",
...
export function OrgQuickNavigate({ totalElections, totalMembers }: OrgQuickNavigateProps) {
  return (
          ...
          {navItems.map((item, index) => {
            ...
            return (
              <Link
                key={item.key}
                href={item.href}
                className={`p-6 hover:bg-muted/40 transition-all border-b sm:border-r sm:[&:nth-child(2n)]:border-r-0 flex items-center justify-between group border-border/50 ${isBottomRow ? "sm:border-b-0" : ""} ${isLast ? "border-b-0" : ""}`}
              >
                <div className="flex items-center gap-4">
                  ...
                  <div className="space-y-0.5">
                    ...
                    <p className="text-[11px] font-bold text-muted-foreground tracking-tight">{item.description}</p>
                  </div>
                </div>
