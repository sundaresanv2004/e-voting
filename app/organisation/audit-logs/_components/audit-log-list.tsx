    ...
    <div className="space-y-4">
      {logs.map((log) => {
        ...
        return (
          <div
            key={log.id}
            className="rounded-3xl border border-border/40 bg-card p-5 shadow-sm flex flex-col sm:flex-row sm:items-start gap-4 justify-between"
          >
            {/* Left: Icon and core details */}
            <div className="flex items-start gap-4 z-10 w-full pl-1">
              <div
                className={`h-10 w-10 shrink-0 rounded-xl flex items-center justify-center border shadow-sm ring-1 ring-inset ring-foreground/5 backdrop-blur-md ${colorClass}`}
              >
              ...
              <div className="space-y-1.5 w-full">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <Badge
                    variant={STATUS_VARIANTS[log.status] || "default"}
                    className="text-[9px] uppercase tracking-wider py-0 px-2 rounded-full h-4 font-black shadow-sm"
                  >
                    {log.status}
                  </Badge>
                </div>

                <div className="flex flex-wrap sm:items-center gap-x-3 gap-y-2 text-xs text-muted-foreground/80 font-semibold">
                  <p className="flex items-center gap-1">
                    <span className="text-muted-foreground/60">Actor:</span>
                    <span className="text-foreground/80 font-bold">{actor}</span>
                  </p>
                  <span className="hidden sm:inline text-border/70">•</span>
                  <p className="tabular-nums font-medium flex items-center gap-1">
                    <HugeiconsIcon icon={Calendar01Icon} className="h-3.5 w-3.5 text-muted-foreground/60" />
                    {format(log.createdAt, "PPP · pp")}
                  </p>
                  {log.ipAddress && (
                    <>
                      <span className="hidden sm:inline text-border/70">•</span>
                      <p className="font-mono text-[10px] text-muted-foreground/60 bg-muted/50 px-1.5 py-0.5 rounded-md border border-border/50">
                        {log.ipAddress}
                      </p>
                    </>
                  )}
                  ...
                  <div className="mt-2 w-full max-w-2xl">
                    <Accordion type="single" collapsible className="w-full">
                      <AccordionItem value="metadata" className="border-none">
                        <AccordionTrigger className="py-2 text-[11px] font-bold text-primary/80 hover:text-primary hover:no-underline transition-colors [&[data-state=open]]:text-primary">
                          Inspect Payload Metadata
                        </AccordionTrigger>
                        <AccordionContent>
                          <div className="mt-1 p-3 text-[11px] font-mono bg-muted/40 border border-border/40 rounded-2xl overflow-x-auto text-muted-foreground max-w-full leading-relaxed shadow-inner">
            ...
            <div className="sm:text-right shrink-0 border-t sm:border-t-0 border-border/30 pt-3 sm:pt-0 z-10 flex flex-col sm:items-end justify-center">
              <span className="text-[9px] font-black font-mono text-muted-foreground/40 uppercase tracking-wider block">
                Target ID
              </span>
              <span className="text-[11px] font-mono text-muted-foreground/70 select-all block mt-1 max-w-[200px] truncate sm:max-w-none bg-muted/30 px-3 py-1 rounded-full border border-border/50">
                {log.entityId || log.id}
              </span>
            </div>
          </div>
